...
int main() {
  ...
  while (std::getline(std::cin, line) && !line.empty()) {
    ...
    if (hasOverflowFlag) {
      std::cerr << temp << "\nFormed lists with exit code 1 and error message in standard error because of overflow in sequence: " << word << std::endl;
      return 1;
    }
  ...
  if (sequences.empty()) {
    ...
    try {
      ...
      std::cerr << temp << "\nFormed lists with exit code 1 and error message in standard error because of overflow in sequence: " << e.what() << '\n';
      return 1;
    }
